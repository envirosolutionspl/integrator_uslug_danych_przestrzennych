import unittest
import random
from baza_linkow import linkifikacja
blinkow = linkifikacja
sampyl = blinkow[0]


from web_service import add_service_v2
import random

class TestWebServicePlugin(unittest.TestCase):

    def test_1(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")
    
    def test_2(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")

    
    def test_3(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_4(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_5(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_6(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_7(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_8(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_9(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_10(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_11(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_12(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_13(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_14(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_15(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_16(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_17(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_18(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_19(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_20(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_21(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_22(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_23(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_24(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")


    def test_25(self):
        # WARUNEK
        x = 1
        zmienna = random.randint(0,344)

        #FUNKCJA
        result = add_service_v2(zmienna)

        self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}, link:{linkifikacja[zmienna]}")



if __name__ == '__main__':
    unittest.main()





# class TestWebServicePlugin(unittest.TestCase):

#     def test(self):
#         # WARUNEK
#         losowa = random.randint(1)
#         x = 1


#         #FUNKCJA
#         result = add_service_v2(14)

#         self.assertEqual(result, x, f"Expected {x}, but got {result} for x={x}")

#     def test_sprawdzanie(self):
#         i = 0
#         while i<24:
#             self.test()
#             i = i+1
# if __name__ == '__main__':
#     unittest.main()

